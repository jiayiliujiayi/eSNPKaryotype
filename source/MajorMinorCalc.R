...
MajorMinorCalc <-
  function(Table, minDP, maxDP, minAF) {
    ...
    newTable =
      data.frame("chr" = newTable$chr,
                 "position" = newTable$position,
                 "AF1" = AF1,
                 "AF2" = AF2)
    frequncyTable <-
      newTable[newTable$AF1 >= minAF, ]
    frequncyTable <-
      frequncyTable[frequncyTable$AF2 >= minAF, ]
    orderedTable =
      Sort_major_minor(data = frequncyTable,
                       col1 = 3,
                       col2 = 4)
    MajorMinor <-
      orderedTable$AF1 / orderedTable$AF2
