...
PlotGenome <-
  function(orderedTable,Window,Ylim,PValue,Organism) {
    ...
    ## Change to hg38 size, need to find centromere positions
    if (Organism == "Human") {
      #hg38
      ...
      # hg19
      # chr_size = c(249250621,243199373,198022430,191154276,180915260,
    ...
    orderedTable$position <-
      orderedTable$position + chr_total[orderedTable$chr]

    col <- rollmedian(orderedTable$chr,window) %% 2 + 1
    col[col == 1] <- "dodgerblue4"
    col[col == 2] <- "dodgerblue1"

    plot(rollmedian(orderedTable$position,
                    window),
         rollmedian(orderedTable$MajorMinor,
                    window),
         col = "dimgrey",
         pch = 15,
         cex = 0.4,
         ...
         xlab = "",
         xaxt = "n",
         xlim = c(1, genome_size))

    points(rollmedian(orderedTable$position,
                      window),
           rollmedian(orderedTable$MajorMinor,
                      window),
           col = col,
    ...
    if (isTRUE(PValue)) {
      ...
      l <- length(rollmedian(orderedTable$MajorMinor,
                             window))
      xx <- rollmedian(orderedTable$position,
                       window)
    ...
    for (i in 1:(length(chr_size) - 1)) {
      ...
      lines(c(sum + 20000000,
              sum + chr_size[i] - 20000000),
            c(1,1),
            lwd = 10,
            col = "black")
      lines(c(sum + 20000000,
      ...
      if (Organism == "Human") {
        lines(c(sum + centromere_pos[i],
                sum + centromere_pos[i]),
              c(1.01, 0.99),
              col = "grey13",
              lwd = 2)
      }
    ...
    lines(c(sum + 20000000,
          ...
          col = "gray90")
    if (Organism == "Human") {
      lines(c(sum + centromere_pos[24],
              sum + centromere_pos[24]),
            c(1.01, 0.99),
            col = "grey13",
            lwd = 2)}
